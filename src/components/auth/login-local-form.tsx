...
export function LoginLocalForm() {
  ...
  function validateLoginFields(): boolean {
    ...
    else if (!emailSchema.safeParse(email.trim()).success) errors.email = "Enter a valid email address.";
  ...
  async function handleEmailLogin(event: React.FormEvent) {
    ...
    try {
      if (!supabaseReady) {
        ...
        await refreshProfile();
        router.push(localUser.onboardingComplete ? (callbackUrl ?? "/dashboard") : "/onboarding");
        return;
      }
