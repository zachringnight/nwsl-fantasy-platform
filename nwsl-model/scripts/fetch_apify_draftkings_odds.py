#!/usr/bin/env python3
"""Fetch current NWSL 3-way moneyline odds from DraftKings through Apify.

DraftKings is the real US sportsbook the model bets against. The NWSL page is
geo/Akamai-blocked, so this routes an Apify web-scraper run through a US
residential proxy. The Apify token is read only from the environment or a
gitignored .env.local and passed via the request; it is never printed.
"""
...
from src.odds.apify_footystats import update_dataset_manifest_odds

UTC = timezone.utc


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch current NWSL DraftKings odds through Apify Web Scraper")
    parser.add_argument("--url", default=DRAFTKINGS_NWSL_MONEYLINE_URL)
    parser.add_argument("--upcoming", default="data/raw/upcoming.csv")
    parser.add_argument("--output", default="data/raw/odds.csv")
    parser.add_argument("--raw-output", default="data/raw/apify_draftkings_odds_raw.json")
    parser.add_argument("--unmatched-output", default="data/raw/apify_draftkings_odds_unmatched.csv")
...
def main() -> None:
    ...
    if not text:
        raise SystemExit("Apify completed but did not return DraftKings NWSL odds text.")

    parsed = parse_draftkings_odds_text(text)
    if parsed.empty:
        raise SystemExit("DraftKings page text did not contain parseable NWSL moneyline rows.")
    ...
    merged_odds = merge_current_odds_contract(existing_odds, contract, sportsbook="DraftKings")
