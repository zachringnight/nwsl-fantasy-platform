...
## Objective
Extend closing-odds history to cover completed matches after May 24, 2026 (May 29 through July 19 in the current dataset) using the direct plain-HTTP OddsPortal mode from packet 03. Execution-only. Best-effort: the lab proceeds on existing coverage if this fails.
...
4. Repair manifest odds fields:
   `python3 -c "from pathlib import Path; import pandas as pd; from src.odds.apify_footystats import update_dataset_manifest_odds; update_dataset_manifest_odds(Path('data/raw/dataset_manifest.json'), pd.read_csv('data/raw/odds.csv'))"`
...
print(f'Post-May-24 close 1x2 coverage: {covered:.1%} of {len(new)} matches')"
```
Expected on success: coverage above 80 percent of the post-May-24 matches. Below that (or fetch failure): DONE_WITH_CONCERNS with the honest number; the report file must exist either way.
